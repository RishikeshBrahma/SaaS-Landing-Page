...
# 1. For local development, using config.py
# app.config.from_object('config.Config') 

# 2. For deployment (like on Render), using environment variables
...
def get_tasks(project_id):
    ...
    try:
        ...
        if tasks:
            ...
            subtask_query = f"SELECT id, content, is_complete, task_id FROM subtasks WHERE task_id IN ({',%s' * len(task_ids)})".lstrip(',')
...
if __name__ == '__main__':
    # Use os.environ.get for port, default to 5000 for local dev
    port = int(os.environ.get('PORT', 5000))
    # For production, debug should be False
    # The 'host' parameter is set to '0.0.0.0' to be accessible from outside the container
    app.run(debug=False, host='0.0.0.0', port=port)
# --- End of app.py ---
# Note: Ensure that the database and environment variables are correctly set up before running this app.